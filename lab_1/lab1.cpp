...
const string path = "../../../trs_labs_/output/lab_1/";

double U(double x) {
    return -(8 / 9.1) * x * exp(x * x);
}
...
void task1() {
    ...
    for (int i = 0; i < n.size(); i++) {
        ofstream file_to_cout_V(path + "z1_v" + to_string(n[i]) + ".txt");
        ofstream file_to_cout_X(path + "z1_x" + to_string(n[i]) + ".txt");
        v_prev = 0.09, x_prev = 0;
        v_next = 0, x_next = 0;
        h = 10. / (n[i] - 1);

        file_to_cout_V << "v" << " " << "t" << endl;
        file_to_cout_X << "x" << " " << "t" << endl;
        double t_cur = 0;
        for (int j = 0; j < n[i]; j++) {
            x_next = v_prev * h + x_prev;
            v_next = U(x_next) * h + v_prev;
            file_to_cout_V << v_prev << " " << t_cur << endl;
            file_to_cout_X << x_prev << " " << t_cur << endl;
            v_prev = v_next;
            x_prev = x_next;
            t_cur += h;
        }
...
void task2() {
    ...
    for (int i = 0; i < n.size(); i++) {
        ofstream file_to_cout_V(path + "z2_v" + to_string(n[i]) + ".txt");
        ofstream file_to_cout_X(path + "z2_x" + to_string(n[i]) + ".txt");
        v_prev_prev = 0.09, x_prev_prev = 0;
        h = 10. / (n[i] - 1);
        x_prev = v_prev_prev * h + x_prev_prev;
        v_prev = U(x_prev_prev) * h + v_prev_prev;
        ...
        file_to_cout_V << v_prev_prev << " " << t_cur << endl;
        file_to_cout_X << x_prev_prev << " " << t_cur << endl;
...
void task3() {
    ...
    for (int i = 0; i < n.size(); i++) {
        ofstream file_to_cout_V(path + "z3_v" + to_string(n[i]) + ".txt");
        ofstream file_to_cout_X(path + "z3_x" + to_string(n[i]) + ".txt");
        v_prev = 0.09, x_prev = 0;
        v_next = 0, x_next = 0;
