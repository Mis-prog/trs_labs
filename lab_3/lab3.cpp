...
const string path = "../../output/lab_3/";
...
void createAnaliticFile(int N, int M) {
    ofstream ftc(path + "analitic.txt");
...
double f_4(double x, double y) {
    return (-6.283185308) * exp(x) * sin(3.141592654 * x) * sin(3.141592654 * y) * cos(3.141592654 * x) -
           19.73920881 * exp(x) * cos(3.141592654 * x) * cos(3.141592654 * x) * sin(3.141592654 * y) +
           19.73920881 * exp(x) * sin(3.141592654 * x) * sin(3.141592654 * x) * sin(3.141592654 * y) -
           3.141592654 * exp(y) * sin(3.141592654 * x) * sin(3.141592654 * x) * cos(3.141592654 * y) +
           9.869604404 * exp(y) * sin(3.141592654 * x) * sin(3.141592654 * x) * sin(3.141592654 * y) -
           1. * sin(3.141592654 * x) * sin(3.141592654 * x) * sin(3.141592654 * y);
}
...
void z1_Poisson_problem_eq(int N, int M, int paramToCout = 0) {
    ...
    while (abs(diff) > 1e-15) {
        ...
        for (int j = 0; j < (M - 1); j++) {
            ...
            for (int i = 0; i < (N - 1); i++) {
                x_next[j * (N - 1) + i] =
                        (f[j * (N - 1) + i] - buf_Mult_MV[j * (N - 1) + i]) / U_matrix_returning(i, i, N, M);
...
void z2_Poisson_problem_eq(int N, int M, int paramToCout = 0) {
    ...
    ofstream ftc_error(path + "task2_error_" + to_string(N) + ".txt");

    double diff_analitic = -1, cur_diff = 0;

    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            cur_diff = abs(u_analitic((i + 1) * hx, (j + 1) * hy) - x_next[j * (N - 1) + i]);
    ...
    if (paramToCout == 1) {
        ofstream ftc(path + "task2_" + to_string(N) + ".txt");
        for (int i = 0; i <= N; i++) {
            ftc << phi3(hx * i) << " "; // border y = 0;
        }
        ftc << endl;
...
void z3(int N, int M, int ptc) {
    ...
    ofstream ftc_error(path + "task3_error_" + to_string(N) + ".txt");

    double diff_analitic = -1, cur_diff = 0;
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            cur_diff = abs(u_analitic((i + 1) * hx, (j + 1) * hy) - solve[j * (N - 1) + i]);
            diff_analitic = max(cur_diff, diff_analitic);
            ftc_error << cur_diff << " ";
        }
        ftc_error << endl;
    }
    ...
    if (ptc == 1) {
        ofstream ftc(path + "task3_" + to_string(N) + ".txt");
        for (int i = 0; i <= N; i++) {
            ftc << phi3(hx * i) << " "; // border y = 0;
        }
        ftc << endl;
...
void z4_Poisson_problem_eq_notwork(int N, int M, int paramToCout = 0) {
    ...
    do {
        ...
        for (int j = 0; j < M - 1; j++) {
            for (int i = 0; i < N - 1; i++) {
                ...
                Az[j * (N - 1) + i] =
                        (1 - 2 * (exp(x_coord) / hx / hx + exp(y_coord) / hy / hy)) * z_prev[j * (N - 1) + i];
                if (i > 0) {
                    Az[j * (N - 1) + i] +=
                            exp(x_coord) * (1. / hx / hx - 1. / 2. / hx) * z_prev[j * (N - 1) + i - 1]; // betta left
                }
                if (i < N - 2) {
                    Az[j * (N - 1) + i] +=
                            exp(x_coord) * (1. / hx / hx + 1. / 2. / hx) * z_prev[j * (N - 1) + i + 1]; // betta right
                }
                if (j < M - 2) {
                    Az[j * (N - 1) + i] +=
                            exp(y_coord) * (1. / hy / hy + 1. / 2. / hy) * z_prev[(j + 1) * (N - 1) + i]; // gamma right
                }
                if (j > 0) {
                    Az[j * (N - 1) + i] +=
                            exp(y_coord) * (1. / hy / hy - 1. / 2. / hy) * z_prev[(j - 1) * (N - 1) + i]; // gamma left
                }
            }
        }
    ...
    ofstream write_result(path + "task4_" + to_string(N) + ".txt");
    ofstream write_error(path + "task4_error_" + to_string(N) + ".txt");
    ...
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            ...
            write_result << x[j * (N - 1) + i] << " ";
            write_error << cur_diff << " ";
        }
        write_result << endl;
        write_error << endl;
    }
...
void z4_Eigen(int N, int M, int paramToCout = 0) {
    ...
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            x_coord = hx * (i + 1);
            y_coord = hy * (j + 1);
            triplets.push_back(Triplet<double>(j * (N - 1) + i, j * (N - 1) + i,
                                               (1 - 2 * (exp(x_coord) / hx / hx + exp(y_coord) / hy / hy)))); // alpha
        }
    }
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 1; i < (N - 1); i++) {
            x_coord = hx * (i + 1);
            y_coord = hy * (j + 1);
            triplets.push_back(Triplet<double>(j * (N - 1) + i, j * (N - 1) + i - 1,
                                               exp(x_coord) * (1. / hx / hx - 1. / 2. / hx))); // betta left
        }
    }
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 2); i++) {
            ...
            y_coord = hy * (j + 1);
            triplets.push_back(Triplet<double>(j * (N - 1) + i, j * (N - 1) + i + 1,
                                               exp(x_coord) * (1. / hx / hx + 1. / 2. / hx))); // betta right
        }
    }
    for (int j = 0; j < (M - 2); j++) {
        for (int i = 0; i < (N - 1); i++) {
            x_coord = hx * (i + 1);
            y_coord = hy * (j + 1);
            triplets.push_back(Triplet<double>(j * (N - 1) + i, (j + 1) * (N - 1) + i,
                                               exp(y_coord) * (1. / hy / hy + 1. / 2. / hy))); // gamma right
        }
    }
    for (int j = 1; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            ...
            y_coord = hy * (j + 1);
            triplets.push_back(Triplet<double>(j * (N - 1) + i, (j - 1) * (N - 1) + i,
                                               exp(y_coord) * (1. / hy / hy - 1. / 2. / hy))); // gamma left
        }
    }
    ...
    ofstream write_result(path + "task4_eigen_" + to_string(N) + ".txt");
    ofstream write_error(path + "task4_eigen_error_" + to_string(N) + ".txt");

    double diff_analitic = -1, curr_dif = 0;
    double max_num = 0;
    for (int j = 0; j < (M - 1); j++) {
        for (int i = 0; i < (N - 1); i++) {
            curr_dif = abs(u_analitic((i + 1) * hx, (j + 1) * hy) - x[j * (N - 1) + i]);
            diff_analitic = max(curr_dif,
                                diff_analitic); // error estimation
            max_num = max(abs(x[j * (N - 1) + i]), max_num);
            write_error << curr_dif << " ";
            write_result << x[j * (N - 1) + i] << " ";
        }
        write_error << endl;
        write_result << endl;
